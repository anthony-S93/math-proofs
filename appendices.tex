...
\begin{rem}
    If \( k = 0 \), then \( \finiteseq{k} = \mleft\{ \emptyset \mright\} \) and \( \delta_{i} = \emptyset \) for all \( i \in \abs{\tau} \);
    additionally, \( \abs{\mathfrak{A}}^{k} = \mleft\{ \emptyset \mright\} \) according to
    ...
    \( \mathcal{L} \)-variable to \aref{dfn:first-order-language-variable-occurrence}{occur in}; this means that \( \delta = \emptyset \) and therefore
    \( \sequencebig{\mleft( \tau_{i} \mright)^{\mathfrak{A}}(\composition{x}{\delta_{i}})}{i \in \abs{\tau}} = \emptyset = \tuple{} \).
    In other words, \( \tau^{\mathfrak{A}}_{\delta} = \mleft\{ \orderedpair{\emptyset}{\tuple{}} \mright\} = \mleft\{ \orderedpair{\emptyset}{\emptyset} \mright\} \).
\end{rem}

\noindent
With \cref{dfn:first-order-language-interpretation-of-a-tuple-of-terms} at our disposal, we are now fully equipped to give a general definition of
the interpretation of an \( \mathcal{L} \)-term in the context of an \( \mathcal{L} \)-structure.
...
\begin{dfn}
    ...
    then \( t^{\mathfrak{A}} \) is defined according to the following rules:
    \begin{enumerate}[topsep=2pt,label=\textup{\texttt{(\alph*)}}]
        ...
        \item If \( t \) is \( F^{\mathcal{L}}_{i}(s) \) for some \( i \in \dom F^{\mathcal{L}} \) where \( s \) is a tuple of \( \mathcal{L} \)-terms such that
            ...
            \aref{dfn:first-order-language-variable-distribution-function}{variable distribution function} of \( s \),
            then,
            \begin{equation*}
                t^{\mathfrak{A}} = \setbuilderBig{\orderedpair{x}{y}}{x \in \abs{\mathfrak{A}}^{k}
                \,\,{\land}\,\,
                y = F^{\mathfrak{A}}_{i}(s^{\mathfrak{A}}_{\delta})
                }
            \end{equation*}
    \end{enumerate}
\end{dfn}
...
\begin{rem}
    For the third rule, if \( k = 0 \), then clearly \( t^{\mathfrak{A}} = \mleft\{ \orderedpair{\emptyset}{F^{\mathfrak{A}}_{i}(s^{\mathfrak{A}})} \mright\} \)
    ...
    \( s_{j} \) (otherwise, \( k  \) would not be \( 0 \)).
