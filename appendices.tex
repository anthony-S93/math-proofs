...
where \( k \in \naturalset \) is the number of variable symbols occurring in the term and \( \tau \) is a tuple of all the
\aref{dfn:first-order-language-terms-root-and-subterms}{subterms} of the compound term.
...
\vspace{0.3cm}
\noindent
This approach trades explicitness and succinctness for maximum generality in that it allows the internal structure of \emph{any} compound first-order term
(even explicit ones) to be specified in virtually any situation.
For instance, even though the internal structure of \( s \) in our example has been explicitly specified as \( \tuple{f(x), f(y), g(x,z)} \),
it is still possible to express it implicitly in terms of a function
\( \function{\delta}{\abs{\tau}}{\finiteseq{3}} \) where \( \delta = \tuple{\tuple{0}, \tuple{1}, \tuple{0,2}} \).
