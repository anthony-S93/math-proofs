...
\vspace{0.3cm}
\noindent
A component of considerable interest in \eqref{eq:interpreted-s-implicit} is the sequence
\( \sequencebig{(\tau_{i})^{\mathfrak{A}}(\composition{\alpha}{\delta_{i}})}{i \in \abs{\tau}} \).
What makes it interesting is that it hints at the possibility of interpreting, not just a single term,
but also a \emph{tuple} of terms in the context of a structure;
i.e., the sequence \( \sequencebig{(\tau_{i})^{\mathfrak{A}}(\composition{\alpha}{\delta_{i}})}{i \in \abs{\tau}} \)
can be thought of as the result of interpreting the tuple of terms \( \tuple{f(x), f(y), g(x,z)} \) in the context of \( \mathfrak{A} \).
The key point here is that \( \tau \) is interpreted \emph{as a whole} rather than separately according to its individual components.

\vspace{0.3cm}
\noindent
In general, any non-empty tuple of \( \mathcal{L} \)-terms can be interpreted as a whole in the context a compatible \( \mathcal{L} \)-structure
as long as two pieces of information are known:
\begin{enumerate*}[label=(\roman*)]
    \item \( k \) the number of \( \mathcal{L} \)-variables occurring in at least one \( \tau_{i} \) for some \( i \in \abs{\tau} \),
    \item the distribution of \( \mathcal{L} \)-variables across all \( \tau \), which can be provided in the form of a function
        \( \function{\delta}{\abs{\tau}}{\finiteseq{k}} \).
\end{enumerate*}
