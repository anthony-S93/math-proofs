...
each symbol in \( \mathcal{L} \) is analogous to a word in a natural language. Just as the words of a natural language can be
stringed together to form expressions, the symbols of \( \mathcal{L} \) can also be stringed together to form \( \mathcal{L} \)-expressions.
...
\begin{dfn}
    ...
    \begin{enumerate}[topsep=2pt,label=\textup{\texttt{(\alph*)}}]
        \item \( t \) is an \( \mathcal{L} \)-variable.
        \item \( t \) is \( F^{\mathcal{L}}_{i}(s) \) where \( i \in \dom F^{\mathcal{L}} \) and:
            ...
            \end{itemize}
        \item \( t \) is \( (s) \) where \( s \) is an \( \mathcal{L} \)-term
        \item \( t \) is \( [\thinspace s \thinspace] \) where \( s \) is an \( \mathcal{L} \)-term
    \end{enumerate}
\end{dfn}
...
\begin{dfn}
    ...
    where \( \tau \) is a non-empty tuple such that \( \tau_{j} \) is an \( \mathcal{L} \)-term containing at least one symbol
    from \( F^{\mathcal{L}} \) for some \( j \in \abs{\tau} \).
...
\emph{meaning} to all valid formulas just as words like ``cats'', ``apples'', and ``oranges'' need to have meaning
...
it provides a connection between a bunch of seemingly meaningless symbols and the real world.
...
In fact, for a first-order structure to be capable of interpreting \( \mathcal{L} \), its \aref{dfn:signature-of-a-structure}{signature} must be
compatible with the arity functions \aref{dfn:first-order-language-symbols}{\( \Pi^{\mathcal{L}} \) and \( \Phi^{\mathcal{L}} \)},
which leads us to the following definition.

\begin{dfn}
    An \( \mathcal{L} \)-structure is a first-order structure with \aref{dfn:signature-of-a-structure}{signature}
...
\begin{rem}
    ...
    In other words, there exist one-to-one correspondences between the function symbols of \( \mathcal{L} \) and the n-ary operation of \( \mathfrak{A} \)
...
\begin{notation}
    ...
    then we write \( t^{\mathfrak{A}} \) to denote the entity obtained via an interpretation of \( t \) in the context
    of \( \mathfrak{A} \).
\end{notation}
...
Moreover, for each \( \alpha \in \abs{\mathfrak{A}}^{k} \), \( t^{\mathfrak{A}}(\alpha) \) is the element
...
\vspace{0.3cm}
\noindent
Notice that in the examples we provided, the internal structures of \( s \) and \( t \) are explicitly defined;
...
\aref{dfn:first-order-language-terms-root-and-subterms}{subterms} (or symbols denoting them) of the compound term.
Since \( \tau \) contains all the subterms of the compound term, it is necessary that \( \abs{\tau} \) correspond to the arity of the
\aref{dfn:first-order-language-terms-root-and-subterms}{root of the compound term}.
...
\vspace{0.3cm}
\noindent
In our example, even though the internal structure of \( s \) has been explicitly specified as \( \tuple{f(x), f(y), g(x,z)} \), it is still possible to express
it implicitly in terms of a function \( \function{\delta}{\abs{\tau}}{\finiteseq{3}} \) where \( \delta = \tuple{\tuple{0}, \tuple{1}, \tuple{0,2}} \).
...
\begin{notation}
    ...
    the elements of \( \ran \tau \), then the resulting function from an interpretation of \( \tau \) in the context of \( \mathfrak{A} \) is denoted as
...
\begin{dfn}
    ...
    if and only if for all \( i \in \abs{\tau} \), \( \abs{\delta_{i}} \) corresponds to the number of \( \mathcal{L} \)-variables occurring in \( \tau_{i} \).
\end{dfn}

\begin{rem}
    If \( k = 0 \), then \( \finiteseq{k} = \mleft\{ \emptyset \mright\} \) and thus \( \forall i\,{\in}\,\abs{\tau} \, \big[ \delta_{i} = \emptyset \big] \).
\end{rem}
...
\noindent
With \cref{dfn:first-order-language-interpretation-of-a-tuple-of-terms} at our disposal, we can now give a general definition of
the interpretation of \( \mathcal{L} \)-terms.

\begin{dfn}
    \phantomsection \label{dfn:first-order-language-recursive-interpretation-of-terms}
    If \( t \) is an \( \mathcal{L} \)-term and \( \mathfrak{A} \) is an \( \mathcal{L} \)-structure,
    then \( t^{\mathfrak{A}} \) is defined by the following rules:
    \begin{enumerate}[topsep=2pt,label=\textup{\texttt{(\alph*)}}]
        \item If \( t \) is \( (s) \) or \( [\thinspace s \thinspace] \) and \( s \) is an \( \mathcal{L} \)-term, then \( t^{\mathfrak{A}} = s^{\mathfrak{A}} \)
        ...
        \item If \( t \) is \( F^{\mathcal{L}}_{i}(s) \) for some \( i \in \dom F^{\mathcal{L}} \) where \( s \) is a tuple of \( \mathcal{L} \)-terms such that
            \( \abs{s} = \Phi^{\mathcal{L}}(i) \) and \( \function{\delta}{\abs{s}}{\finiteseq{k}} \) is the variable distribution function of \( s \),
            \begin{equation*}
                t^{\mathfrak{A}} = \setbuilderBig{\orderedpair{x}{y}}{x \in \abs{\mathfrak{A}}^{k}
                \,\,{\land}\,\,
                y = F^{\mathfrak{A}}_{i}(s^{\mathfrak{A}})
                }
            \end{equation*}
    \end{enumerate}
\end{dfn}
