...
will result in a function \( \function{s^{\mathfrak{A}}}{\abs{\mathfrak{A}}^3}{\abs{\mathfrak{A}}} \).
...
\vspace{0.3cm}
\noindent
Even though the term \( s \) in our example has an explicit internal structure \( \tuple{f(x), f(y), g(x,z)} \), it is still possible to express
the inner structure of \( s \) implicitly in terms of a function \( \function{\delta}{\abs{\tau}}{\finiteseq{3}} \):
\begin{align*}
    \delta &= \tuple{\tuple{0}, \tuple{1}, \tuple{0,2}}
\end{align*}
Since \( h \) is the \aref{dfn:first-order-language-terms-root-and-subterms}{root} of \( s \),
the interpreted term \( s^{\mathfrak{A}} \) can be expressed in terms of \( k \), \( \tau \), and \( \delta \) as follows:
\begin{equation}
...
First of all, note that \( k = 3 = \mleft\{ 0, 1, 2 \mright\} \).
...
\vspace{0.3cm}
\noindent
One component of considerable interest in \eqref{eq:interpreted-s-implicit} is the sequence
\( \sequencebig{(\tau_{i})^{\mathfrak{A}}(\composition{\alpha}{\delta_{i}})}{i \in \abs{\tau}} \),
which suggests the possibility of interpreting a \emph{tuple} of terms in the context of a structure;
