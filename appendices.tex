...
`\( z \)', and `\( ) \)'.
This is the most common and direct way to specify the internal structure of \aref{dfn:first-order-language-terms-simple-and-compound-terms}{compound terms}.

\vspace{0.3cm}
\noindent
Nevertheless, there are some situations\footnote{\,In recursive definitions, etc.} in which explicit definitions of subterms are neither practical nor viable.
In such situations, the internal structure of a compound term can be specified \emph{implicitly} via the following parameters:
\begin{enumerate*}[label=(\alph*)]
    \item \( k \in \naturalset \) denoting the number of variable symbols occurring in the term,
    \item \( \tau \) denoting a tuple of all the \aref{dfn:first-order-language-terms-root-and-subterms}{subterms} of the term,
    \item a function \( \function{\delta}{\abs{\tau}}{\finiteseq{k}} \) describing the distribution of variable symbols across the subterms.
\end{enumerate*}

\vspace{0.3cm}
\noindent
The parameters \( k \), \( \tau \), and \( \delta \) must also satisfy the following conditions:
\begin{enumerate*}[label=(\alph*)]
    \item \( \abs{\tau} \) must be equal to the arity of the \aref{dfn:first-order-language-terms-root-and-subterms}{root of the term},
    \item \( \Union_{i \,\in\, \abs{\tau}} \ran \delta_{i} = k \),
    \item for all \( i \in \abs{\tau} \), \( \abs{\delta_{i}} = m_{i} \) where \( m_{i} \) is the number of variable symbols occuring in the subterm \( \tau_{i} \).
\end{enumerate*}

\vspace{0.3cm}
\noindent
In our example, the term \( s \) has an internal structure \( \tuple{f(x), f(y), g(x,z)} \).
We can equivalently express this fact in terms of the parameters \( k \), \( \tau \), and \( \delta \) like so:
\begin{align*}
    k &= 3\\
    \tau &= \tuple{f(x), f(y), g(x, z)}\\
    \delta &= \tuple{\tuple{0}, \tuple{1}, \tuple{0,2}}
\end{align*}
Since \( h \) is the \aref{dfn:first-order-language-terms-root-and-subterms}{root} of \( s \),
\( s^{\mathfrak{A}} \) can be expressed in terms of \( k \), \( \tau \), and \( \delta \) as follows:
\begin{equation}
...
if one were to interpret the term \( h(f(x), f(y), g(x, z)) \) in the context of \( \mathfrak{A} \).
First of all, note that \( k = 3 = \mleft\{ 0, 1, 2 \mright\} \). Thus, for any \( \alpha \in \abs{\mathfrak{A}}^{k} \), \( \alpha = \tuple{a,b,c} \)
...
\end{align*}
Since \( \tau_{0} \), \( \tau_{1} \), and \( \tau_{2} \) are simple, non-compound terms containing only one function symbol,
it follows that \( (\tau_{0})^{\mathfrak{A}} = (\tau_{1})^{\mathfrak{A}} = f^{\mathfrak{A}} \) and \( (\tau_{2})^{\mathfrak{A}} = g^{\mathfrak{A}} \).
This allows us to delineate the components of the internal sequence in \eqref{eq:interpreted-s-implicit}:
\begin{equation*}
...
\vspace{0.3cm}
\noindent
One component of note in \eqref{eq:interpreted-s-implicit} is the sequence \( \sequencebig{(\tau_{i})^{\mathfrak{A}}(\composition{\alpha}{\delta_{i}})}{i \in \abs{\tau}} \),
which can be thought of as the values of each interpreted subterm collected into a tuple.

\vspace{0.3cm}
\noindent
The following definitions formalize the intuitive discussion above.
\begin{dfn}
    Let \( \tau \) be a tuple of \( \mathcal{L} \)-terms. 
    Let \( k \in \naturalset \) be the number of \( \mathcal{L} \)-variables occurring in at least one \( \tau_{j} \) where \( j \in \abs{\tau} \).
    If \( \mathfrak{A} \) is an \( \mathcal{L} \)-structure and \( \function{\delta}{\abs{\tau}}{\finiteseq{k}} \)
    is a function such that for all \( j \in \abs{\tau} \), \( \abs{\delta_{j}} \) corresponds to the number of \( \mathcal{L} \)-variables occurring in
    \( \tau_{j} \), then we define the interpretion of \( s \) under \( \mathfrak{A} \) as the function
    \( \function{s^{\mathfrak{A}}_{\delta}}{\abs{\mathfrak{A}}^{k}}{\abs{\mathfrak{A}}^{n}} \) such that:
    \begin{equation*}
        s^{\mathfrak{A}}_{\delta} = \setbuilderBig{\orderedpair{x}{y}}{x \in \abs{\mathfrak{A}}^{k} \,\,{\land}\,\,
        y = \tuple{s^{\mathfrak{A}}_{0}(\composition{x}{\delta_{0})}, \dots, s^{\mathfrak{A}}_{n-1}(\composition{x}{\delta_{n-1}) }}}
    \end{equation*}
\end{dfn}

\begin{rem}
    If \( k = 0 \), then \( \finiteseq{k} = \mleft\{ \emptyset \mright\} \) and thus \( \delta_{i} = \emptyset \) for all \( i \in n \).
    If \( s \) is the empty tuple, then necessarily \( k = 0 \) because there aren't any \( \mathcal{L} \)-terms in \( s \) for any
    \( \mathcal{L} \)-variable to \aref{dfn:first-order-language-variable-occurrence}{occur in}.
\end{rem}

\vspace{0.3cm}
\noindent
If \( s \) is a \emph{tuple} of \( \mathcal{L} \)-terms with length \( n \in \naturalset \),
then \( s^{\mathfrak{A}} \) denotes the tuple containing all interpreted \( \mathcal{L} \)-terms in \( s \); i.e.,
\( \tuple{s^{\mathfrak{A}}_{0}, \dots, s^{\mathfrak{A}}_{n-1}} \).
Note that \( s \) could be the \( 0 \)-tuple \( \tuple{ } \), in which case \( s^{\mathfrak{A}} = \emptyset \).

\noindent
As stated in the \aref{dfn:first-order-language-terms}{syntax of \( \mathcal{L} \)}, an \( \mathcal{L} \)-term can contain
\( \mathcal{L} \)-variables. Thus, if \( s \) is a tuple of \( \mathcal{L} \)-terms, then it is reasonable to expect 
\( s \) to ``contain'' a certain number of \( \mathcal{L} \)-variables; i.e., an \( \mathcal{L} \)-variable \( v \)
is contained in \( s \) if and only if there exists some \( j \in \abs{s} \) such that \( v \)
\aref{dfn:first-order-language-variable-occurrence}{occurs in} \( s_{j} \).
...
\begin{dfn}
    ...
    \begin{enumerate}[topsep=2pt,label=\textup{\texttt{(\alph*)}}]
        ...
        \item If \( t \) is \( F^{\mathcal{L}}_{i}(s) \) for some \( i \in \dom F^{\mathcal{L}} \) and \( s \) is a tuple 
            of \( \mathcal{L} \)-terms with \( \abs{s} = \Phi^{\mathcal{L}}(i) \)
            and \( k \) is the number of \( \mathcal{L} \)-variables occurring in \( t \), then,
            \begin{equation*}
