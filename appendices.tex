...
We further note that the function \( \delta \) is just as much a part of the interpretation as the structure \( \mathfrak{A} \); i.e., a different \( \delta \)
will yield a completely different interpretation. To emphasize this fact, we shall utilize the following notational convention.
...
\begin{dfn}
    \phantomsection \label{dfn:first-order-language-variable-distribution-function}
    If \( \tau \) is a tuple of \( \mathcal{L} \)-terms and \( k \in \naturalset \) is the number
    of \( \mathcal{L} \)-variables occurring in at least one \( \tau_{j} \) where \( j \in \abs{\tau} \),
    then a function \( \function{\delta}{\abs{\tau}}{\finiteseq{k}} \) is called a \keyword{variable distribution function} of \( \tau \)
    if and only if for all \( i \in \abs{\tau} \), \( \abs{\delta_{i}} \) corresponds to the number of \( \mathcal{L} \)-variables occurring in
    the term \( \tau_{i} \).
\end{dfn}

\begin{dfn}
    \phantomsection \label{dfn:first-order-language-interpretation-of-a-tuple-of-terms}
    Let \( \tau \) be a tuple of \( \mathcal{L} \)-terms and \( k \in \naturalset \) be the number of \( \mathcal{L} \)-variables
    occurring in at least one \( \tau_{j} \) where \( j \in \abs{\tau} \).
    If \( \mathfrak{A} \) is an \( \mathcal{L} \)-structure and \( \function{\delta}{\abs{\tau}}{\finiteseq{k}} \)
    is a \aref{dfn:first-order-language-variable-distribution-function}{variable distribution function} of \( \tau \),
    then we define the interpretion of \( \tau \) in the context of \( \mathfrak{A} \) and \( \delta \) as the function
    \( \function{\tau^{\mathfrak{A}}_{\delta}}{\abs{\mathfrak{A}}^{k}}{\abs{\mathfrak{A}}^{\abs{\tau}}} \) such that:
    \begin{equation*}
