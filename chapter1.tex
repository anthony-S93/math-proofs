...
\begin{proof}
    ...
    \end{equation}
    Let \( x_{0} \) be an arbitrary object. Then by \eqref{eq:A-B-empty}, \( x_{0} \notin A \,\,{\land}\,\, x_{0} \notin B \).
...
\begin{thm}[Restricted Comprehension]
    ...
    If \( \varphi(x) \) is a \aref{dfn:first-order-language-construction-of-formulas}{formula} of some first-order language of set theory,
...
\begin{proof}
    ...
    \end{gather}
    Let \( x_{0} \) be arbitrary.
    Suppose that \( x_{0} \in S \).
    Thus, \aref{rule-of-inference:universal-instantiation}{instantiating} \eqref{eq:S-satisfies-phi} immediately
    yields \( x_{0} \,{\in}\, A \,\,{\land}\,\, \varphi(x_{0}) \), which in turn implies \( x_{0} \in S^{\prime} \) by \eqref{eq:S-prime-satisfies-phi}.
...
\begin{thm}
    ...
    If \( \varphi(x) \) is a formula of some first-order language of set theory, then
    \begin{equation*}
