...
\begin{dfn}
    The term \emph{formula} refers to a well-formed formula of a \aref{dfn:first-order-language}{first-order language}.
    Unless otherwise specified, the reader should always assume that a formula has exactly one free variable.
\end{dfn}

\begin{thm}[Restricted Comprehension]
    \label{thm:uniqueness-predicate-set}
    Given any formula \( \varphi \) and any set \( A \), the set \( S \) such that
