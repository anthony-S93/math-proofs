...
\end{proof}
\noindent
Since the empty set is unique, we can denote it with a single universal symbol.
\begin{dfn}
    \label{dfn:empty-set}
    \( \emptyset \) denotes the empty set; in other words, \(  \forall x \, ( x \notin \emptyset ) \).
\end{dfn}

\begin{thm}[Restricted Comprehension]
    \label{thm:uniqueness-predicate-set}
    Let \( \varphi \) be a \aref{dfn:first-order-language-construction-of-formulas}{formula} with a single free variable.
...
\begin{proof}
    ...
    Then \aref{rule-of-inference:universal-instantiation}{Instantiating} \eqref{eq:S-satisfies-phi}
    yields \( x_{0} \,{\in}\, A \,\,{\land}\,\, \varphi(x_{0}) \). By \eqref{eq:S-prime-satisfies-phi}, it follows that \( x_{0} \in S^{\prime} \).
    Thus, \( x_{0} \in S {\implies} x_{0} \in S^{\prime} \) by the \aref{rule-of-inference:deduction-theorem}{Deduction Theorem}. The converse
    can be proven in a similar way, and we thus have \( x_{0} \in S {\iff} x_{0} \in S^{\prime} \). Since \( x_{0} \) is arbitrary,
    we have by \aref{rule-of-inference:universal-generalization}{Universal Generalization},
    \begin{equation*}
...
\begin{thm}
    ...
    For any formula \( \varphi \) with a single free variable,
    \begin{equation*}
...
\begin{proof}
    Let \( \varphi \) be any first-order formula such that,
    \begin{equation}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \begin{aligned}
            ...
            &\quad \text{Definition of Conditional}\\
            &\quad \text{\aref{rule-of-inference:de-morgans-laws}{De Morgan's Laws}}\\
            &\quad \text{\cref{logical-equivalence:grouping-disjuncts-into-universally-quantified-formula}}\\
            &\quad \text{\cref{logical-equivalence:grouping-disjuncts-into-universally-quantified-formula}}\\
            &\quad \text{Definition of Conditional}\\
            &\quad \text{\cref{tautology:using-a-universally-quantified-formula-to-express-the-fact-that-an-object-satisfies-a-formula}}\\
            &\quad \text{\cref{dfn:difference}}\\
    ...
    \begin{equation*}
        ...
        \begin{aligned}
            ...
            &\quad \text{\cref{tautology:grouping-conjuncts-into-existentially-quantified-formula}}\\
            &\quad \text{\cref{tautology:grouping-conjuncts-into-existentially-quantified-formula}}\\
