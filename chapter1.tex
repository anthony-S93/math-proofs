...
Many properties of sets follow directly from the axioms listed in \cref{ch:zfc-axioms}. Note that
...
Therefore, we begin this chapter by proving the uniqueness of the various sets postulated by the basic axioms.
...
\begin{proof}
    Let \( A \) and \( B \) be any two sets that do not contain any elements.
    We have to show that necessarily \( A = B \).

    \vspace{0.3cm}
    \noindent
    Let \( x_{0} \) be an arbitrary set.
    Since neither \( A \) nor \( B \) contain any elements, \( x_{0} \) cannot possibly belong to either \( A \) or \( B \);
    that is, \( x_{0} \notin A \) and \( x_{0} \notin B \) are both true.
    Consequently, both \( x_{0} \in A {\implies} x_{0} \in B \) and \( x_{0} \in B {\implies} x_{0} \in A \) are vacuously true.
    By \aref{rule-of-inference:biconditional-introduction}{Biconditional Introduction}, we derive \( \mleft( x_{0} \in A {\iff} x_{0} \in B \mright) \).
...
\begin{notation}
    \label{notation:empty-set}
    Since the empty set is unique, it is justified to denote with a unique symbol, \( \emptyset \).
\end{notation}

\begin{dfn}[Empty Set]
    \phantomsection \label{dfn:empty-set}
    \( \forall x \, \mleft( x \notin \emptyset \mright) \)
\end{dfn}

\begin{thm}[Restricted Comprehension]
    \label{thm:uniqueness-predicate-set}
    If \( \varphi \) is a \aref{dfn:first-order-language-construction-of-formulas}{formula} with a single free variable,
    then for any set \( A \), the set \( S \) such that \( \forall x \, \mleft[ x \in S {\iff} x \in A \,\,{\land}\,\, \varphi(x) \mright] \) is unique.
\end{thm}

\begin{proof}
    Let \( S \) and \( S^{\prime} \) be sets such that,
    \begin{gather}
        \forall x \, \big[ x \in S \iff x \in A \,\,{\land}\,\, \varphi(x) \big] \label{eq:S-satisfies-phi} \\
        \forall x \, \big[ x \in S^{\prime} \iff x \in A \,\,{\land}\,\, \varphi(x) \big] \label{eq:S-prime-satisfies-phi}
    \end{gather}
    Note that the \nameref{axm:axiom-schema-of-specification} guarantees the existence of \( S \) and \( S^{\prime} \).
    Let \( x_{0} \) denote an arbitrary set such that \( x_{0} \in S \).
    Thus, \aref{rule-of-inference:universal-instantiation}{instantiating} \eqref{eq:S-satisfies-phi} immediately
    yields \( x_{0} \,{\in}\, A \,\,{\land}\,\, \varphi \), which in turn implies \( x_{0} \in S^{\prime} \) by \eqref{eq:S-prime-satisfies-phi}.
    Since \( x_{0} \in S^{\prime} \) is derivable from the supposition that \( x_{0} \in S \), we have by the \aref{rule-of-inference:deduction-theorem}{Deduction Theorem}
    \begin{equation*}
        x_{0} \in S {\implies} x_{0} \in S^{\prime}
    \end{equation*}
    The converse can be proven in a similar fashion, and we thus have \( x_{0} \in S {\iff} x_{0} \in S^{\prime} \).
    Given that \( x_{0} \) is arbitrary, we have by \aref{rule-of-inference:universal-generalization}{Universal Generalization}
    \begin{equation*}
...
\begin{thm}
    ...
    If \( \varphi \) is a formula in some first-order language of set theory, then
    \begin{equation*}
        \exists A \, \forall x \, \big[ \varphi \implies x \in A \big]
        \implies
        \exists! S \, \forall x \, \big[ x \in S \iff \varphi \big]
    \end{equation*}
\end{thm}

\begin{proof}
    Let \( \varphi \) be a formula in any first-order language of set theory such that
    \begin{equation}
        \label{eq:phi-x-implies-x-in-A}
        \forall x \, ( \varphi \implies x \in A )
    \end{equation}
    holds for some set \( A \).
    ...
    \begin{equation}
        ...
        \forall x \, \mleft( x \in S \iff x \in A \,\,{\land}\,\, \varphi \mright)
    \end{equation}
    Let \( x_{0} \in S \) be arbitrary. Thus \( x_{0} \in A \,\,{\land}\,\, \varphi \) follows immediately
    from \eqref{eq:property-of-S}.
    By \aref{rule-of-inference:conjunction-elimination}{Conjunction Elimination}, we can then infer \( \varphi(x_{0}) \).
    \begin{equation}
    ...
    And by \aref{rule-of-inference:conjunction-introduction}{Conjunction Introduction}, we infer \( x_{0} \in A \land \varphi(x_{0}) \),
    ...
    \end{equation}
    By \eqref{eq:phi-x-implies-x-in-S}, \eqref{eq:x-in-S-implies-phi-x}, and \aref{rule-of-inference:universal-generalization}{Universal Generalization},
    \begin{equation*}
        \forall x \, \mleft( x \in S \iff \varphi(x) \mright)
    \end{equation*}
    Thus, \( \exists  S \, \forall x \, \mleft( x \in S \iff \varphi(x) \mright) \) by \aref{rule-of-inference:existential-generalization}{Existential Generalization}.
...
\begin{rem}
    \cref{thm:uniqueness-predicate-set,thm:sufficient-condition-for-unrestricted-comprehension} serve as the rigorous foundation for the familiar set-builder notation.
\end{rem}
...
\begin{proof}
    ...
    \eqref{eq:phi1-iff-phi2} and \cref{dfn:set-builder-notation}. Since \( x_{0} \) is arbitrary, \aref{rule-of-inference:universal-generalization}{Universal Generalization} yields,
    \begin{equation}
...
\begin{rem}
    ...
    Thus, we can invoke the following chain of inferences:
    \begin{equation*}
        \begin{aligned}
        ...
        \text{  }
        \begin{aligned}
            &\quad\text{\aref{rule-of-inference:assumption}{Assumption}}\\
            ...
            &\quad\text{\aref{rule-of-inference:disjunctive-syllogism}{Disjunctive Syllogism}}\\
            &\quad\text{\aref{rule-of-inference:conjunction-elimination}{Conjunction Elimination}}
        \end{aligned}
    \end{equation*}
    ...
    \begin{equation*}
        ...
        \begin{aligned}
            &\quad\text{\aref{rule-of-inference:assumption}{Assumption}} \\
            &\quad\text{by \eqref{eq:phi1-definition}} \\
            &\quad\text{by \eqref{eq:A-B-no-common-elements} and \aref{rule-of-inference:universal-instantiation}{Universal Instantiation}} \\
            &\quad\text{\aref{rule-of-inference:modus-tollens}{Modus Tollens}} \\
            &\quad\text{\aref{rule-of-inference:disjunction-introduction}{Disjunction Introduction}} \\
            &\quad\text{\aref{rule-of-inference:de-morgans-laws}{De Morgan's Laws}} \\
            &\quad\text{\aref{rule-of-inference:conjunction-introduction}{Conjunction Introduction}} \\
            &\quad\text{\aref{rule-of-inference:de-morgans-laws}{De Morgan's Laws}} \\
            ...
            &\quad\text{\aref{rule-of-inference:existential-generalization}{Existential Generalization}} \\
...
\begin{proof}
    ...
    Since \( x_{0} \) is arbitrary, we conclude from \aref{rule-of-inference:universal-generalization}{Universal Generalization} that,
    \begin{equation*}
        \forall x \, \mleft( x \notin S \mright)
    \end{equation*}
...
\begin{proof}
    ...
    \end{equation*}
    By \cref{tautology:using-an-existentially-quantified-formula-to-express-the-fact-that-an-object-satisfies-a-formula}, this is equivalent to,
    \begin{equation*}
        \forall y \, \big( y \in A \implies y \neq A \big) \iff \forall y \, \big( y = A \implies y \notin A \big)
    \end{equation*}
    By \aref{rule-of-inference:universal-instantiation}{Universal Instantiation}, we thus have,
    \begin{equation*}
        A = A \implies A \notin A
    \end{equation*}
    Since \( A = A \) is trivially true, we conclude \( A \notin A \) by \aref{rule-of-inference:modus-ponens}{Modus Ponens}. Since \( A \) is arbitrary, the theorem follows immediately
    from \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
\end{proof}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \end{aligned}
    \end{equation*}
    Since \( A \) is arbitrary, the theorem follows from \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
\end{proof}
...
\begin{proof}
    ...
    \end{equation*}
    By \eqref{eq:apply-definition-of-power-set} and \aref{rule-of-inference:universal-instantiation}{Universal Instantiation},
    \begin{equation*}
...
\begin{proof}
    ...
    \end{equation}
    From elementary logic the \aref{rule-of-inference:law-of-excluded-middle}{Law of Excluded Middle}, we know that,
    \begin{equation}
    ...
    S \,{\in}\, S \), a logical impossibility. By Reductio ad Absurdum, we conclude that
    \( S \notin S \) cannot possibly be true. Thus, by \aref{rule-of-inference:disjunctive-syllogism}{Disjunctive Syllogism}, we conclude that,
    \begin{equation*}
        S \in S
    \end{equation*}
    ...
    had to be false, which is impossible by the \aref{rule-of-inference:law-of-excluded-middle}{Law of Excluded Middle}.
...
\begin{dfn}[Subset]
    \label{dfn:subset}
    Given any set \( B \), a set \( A \) is a \keyword{subset} of \( B \) (denoted as \( A \subseteq B \))
    if and only if \( \forall x \, \big[ x \in A {\implies} x \in B \big] \).
\end{dfn}

\begin{cor}
    \phantomsection \label{cor:A-equals-B-implies-A-subset-B}
    \( \forall A \, \forall B \, \big[ A = B {\implies} A \subseteq B \big] \).
\end{cor}

\begin{dfn}[Proper Subset]
    \label{dfn:proper-subset}
    Given any set \( A \), a set \( B \) is a \keyword{proper subset} of A
    (denoted as \( B \subset A \)) if and only if \( B \subseteq A \,\,{\land}\,\, B \neq A \).
\end{dfn}

\begin{thm}
    \phantomsection \label{thm:subset-in-terms-of-proper-subset}
    For any two sets \( A \) and \( B \), \( A \subseteq B {\iff} (A \subset B) \,\,{\lor}\,\, (A = B) \).
\end{thm}

\begin{proof}
    Let \( A \) and \( B \) be arbitrary sets. By \cref{dfn:proper-subset},
    \begin{align*}
        A \subset B &\iff A \subseteq B \,\,{\land}\,\, A \neq B\\
        \therefore \, (A \subset B) \,\,{\lor}\,\, (A = B) &\iff \mleft( A \subseteq B \,\,{\land}\,\, A \neq B \mright) \,\,{\lor}\,\, (A = B)
    \end{align*}
    By \aref{rule-of-inference:distribution}{Distribution}, the second formula becomes,
    \begin{align}
        \mleft( A \subset B \mright) \,\,{\lor}\,\, \mleft( A = B \mright) &\iff \mleft( A \subseteq B \,\,{\lor}\,\, A = B \mright)
        \,\,{\land}\,\,
        \mleft( A \neq B \,\,{\lor}\,\, A = B \mright) \notag \\
        &\iff \mleft( A \subseteq B \mright) \,\,{\lor}\,\, \mleft( A = B \mright) \label{eq:A-equals-B-distributed}
    \end{align}
    By \cref{cor:A-equals-B-implies-A-subset-B}, it follows that \( (A = B) {\implies} (A \subseteq B) \).
    Clearly, \( A \subseteq B {\implies} (A \subseteq B) \,\,{\lor}\,\, (A = B) \) by \aref{rule-of-inference:disjunction-introduction}{Disjunction Introduction}.
    The truth of the converse is also quite straightforward. Since \( \mleft( A = B \mright) {\implies} \mleft( A \subseteq B \mright) \), naturally
    \( \mleft( A \subseteq B \mright) \,\,{\lor}\,\, \mleft( A = B \mright) {\implies} A \subseteq B \)
    by \aref{rule-of-inference:disjunction-elimination}{Disjunction Elimination}.
    Finally, by \aref{rule-of-inference:biconditional-introduction}{Biconditional Introduction}, we derive,
    \begin{equation*}
        A \subseteq B {\iff} \mleft( A \subseteq B \mright) \,\,{\lor}\,\, \mleft( A = B \mright)
    \end{equation*}
    Thus, \eqref{eq:A-equals-B-distributed} becomes \( \mleft( A \subset B \mright) \,\,{\lor}\,\, \mleft( A = B \mright) \iff A \subseteq B \). Since
    \( A \) and \( B \) are arbitrary, the theorem follows immediately from \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
\end{proof}
...
\begin{dfn}[Union]
    ...
    \begin{equation*}
        \Union S = \setbuilderBig{x}{\exists \, T \, \mleft( T \in S \,\,{\land}\,\, x \in T \,\mright)}
    \end{equation*}
\end{dfn}

\begin{notation}
    If \( S = \mleft\{ A, B \mright\} \), then \( \Union S \) can be denoted as \( A \union B \).
\end{notation}

\begin{thm}
    \label{thm:union-in-terms-of-logical-or}
    Let \( A \) and \( B \) be arbitrary sets. Then \( A \union B = \setbuilderbig{x}{x \in A \lor x \in B} \). In other words,
    \(  \forall A \, \forall B \, \forall x \, \mleft( x \in A \union B {\iff} x \in A \,\,{\lor}\,\, x \in B \mright) \)
\end{thm}
...
\begin{proof}
    ...
    \end{equation*}
    By \cref{dfn:unordered-pair}, we know that \( T \in \mleft\{ A, B \mright\} {\iff} \mleft( T = A \mright) \,\,{\lor}\,\, \mleft( T = B \mright) \).
    Thus, for any \( x \),
    \begin{align}
    ...
    \end{align}
    By \aref{rule-of-inference:existential-generalization}{Existential Generalization} and
    \cref{tautology:using-an-existentially-quantified-formula-to-express-the-fact-that-an-object-satisfies-a-formula},
    it follows that,
    \begin{align}
    ...
    \end{align}
    Since \( x \) is arbitrary, we have by \aref{rule-of-inference:universal-generalization}{Universal Generalization},
    \begin{equation*}
        \forall x \, \Big[\, \exists \, T \, \big( T \in \{ A,B \} \,\,{\land}\,\, x \in T \big)
        \iff x \in A \,\,{\lor}\,\, x \in B \,\Big]
    \end{equation*}
    The theorem follows immediately from \cref{thm:equal-predicate-specification}.
\end{proof}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \begin{aligned}
            ...
            &\quad\text{\aref{rule-of-inference:existential-instantiation}{Existential Instantiation}, \eqref{eq:A-has-at-least-one-elem}} \\
            &\quad\text{\aref{rule-of-inference:universal-instantiation}{Universal Instantiation}} \\
            &\quad\text{\aref{rule-of-inference:modus-ponens}{Modus Ponens}} \\
            &\quad\text{\aref{rule-of-inference:conjunction-introduction}{Conjunction Introduction}} \\
            &\quad\text{\aref{rule-of-inference:existential-generalization}{Existential Generalization}} \\
            &\quad\text{\cref{dfn:union-of-sets}} \\
            &\quad\text{\aref{rule-of-inference:deduction-theorem}{Deduction Theorem}} \\
            &\quad\text{\aref{rule-of-inference:universal-generalization}{Universal Generalization}} \\
        \end{aligned}
    \end{equation*}
...
\begin{dfn}[Intersection]
    ...
    the \keyword{intersection} of \( S \).
\end{dfn}

\begin{notation}
     If \( S = \mleft\{A, B\mright\} \), we can denote \( \Intersect S \) as \( A \intersect B \).
\end{notation}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \begin{aligned}
            ...
            &\quad\text{\nameref{axm:axiom-of-extensionality}}\\
            &\quad\text{\aref{rule-of-inference:deduction-theorem}{Deduction Theorem}}\\
            &\quad\text{\aref{rule-of-inference:disjunction-elimination}{Disjunction Elimination}}\\
            &\quad\text{\aref{rule-of-inference:deduction-theorem}{Deduction Theorem}}\\
            &\quad\text{\aref{rule-of-inference:universal-generalization}{Universal Generalization}}\\
            &\quad\text{by \eqref{eq:introduce-the-predicate-symbol}}\\
            &\quad\text{\aref{rule-of-inference:deduction-theorem}{Deduction Theorem}}
...
\begin{proof}
    ...
    \end{equation}
    Clearly, \eqref{eq:introduce-the-predicate-for-diff} implies \( \forall x \, \mleft( \varphi(x) {\implies} x \in A \mright) \).
    Thus, \cref{thm:sufficient-condition-for-unrestricted-comprehension} guarantess the existence of a unique set
    \( S \) such that \( \forall x \, \mleft( x \in S {\iff} x \in A \,\,{\land}\,\, x \notin B \mright) \).
    By \aref{rule-of-inference:existential-generalization}{Existential Generalization},
    \begin{gather*}
        \exists! S \, \forall x \, \mleft( x \in S \iff x \in A \land x \notin B \mright)
    \end{gather*}
...
\begin{proof}
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            \therefore \, \forall x \, \mleft( \varphi(x) \implies
            x \in A \union B \mright)&
        \end{aligned}
        \text{  }
        \begin{aligned}
            &\quad\text{\aref{rule-of-inference:assumption}{Assumption}}\\
            &\quad\text{\eqref{eq:introduce-predicate-for-symmetric-difference},
            \aref{rule-of-inference:conjunction-elimination}{Conjunction Elimination}}\\
...
\begin{proof}
    ...
    is vacuously true. By \aref{rule-of-inference:universal-generalization}{Universal Generalization},
    \begin{equation*}
...
\begin{proof}
    ...
    Using elementary propositional logic, we can invoke the following inferences:
    \begin{equation*}
        \begin{aligned}
            x \notin A \lor x \in A& \\
        ...
        \text{  }
        \begin{aligned}
            &\quad\text{\aref{rule-of-inference:law-of-excluded-middle}{Law of Excluded Middle}}\\
            &\quad\text{Logical Equivalence}\\
            &\quad\text{\aref{rule-of-inference:universal-generalization}{Universal Generalization}}
        \end{aligned}
    \end{equation*}
...
\begin{thm}
    \label{thm:only-the-empty-set-can-be-a-subset-of-the-empty-set}
    For any set \( A \), \( A \subseteq \emptyset \) if and only if \( A = \emptyset \).
\end{thm}

\begin{proof}
    It is obvious from \cref{thm:empty-set-universal-subset} that \( A = \emptyset \implies A \subseteq \emptyset \),
    so it remains to prove the converse. Let \( A \) be any subset of \( \emptyset \). Thus, by \cref{dfn:subset}, we
    have,
    \begin{equation}
        \label{eq:x-in-A-implies-x-in-empty}
        \forall x \, \big[ x \in A \implies x \in \emptyset \big]
    \end{equation}
    Suppose that \( A \neq \emptyset \). That is, there exists some \( x_{0} \in A \).
    But if that were true, then \( x_{0} \in \emptyset \) by \eqref{eq:x-in-A-implies-x-in-empty},
    an obvious contradiction. By \aref{rule-of-inference:reductio-ad-absurdum}{Reductio ad Absurdum}, we conclude that \( A = \emptyset \).
    Thus, \( A \subseteq \emptyset \implies A = \emptyset \).
\end{proof}
...
\begin{proof}
    ...
    \end{equation*}
    Since \( \alpha \) is arbitrary, we have \( \forall x \, \mleft( x \in z_{0} \implies x \in B \mright) \) by \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \begin{aligned}
            ...
            &\quad\text{\aref{rule-of-inference:universal-instantiation}{Universal Instantiation}} \\
            &\quad\text{\aref{rule-of-inference:universal-instantiation}{Universal Instantiation}} \\
            &\quad\text{\aref{rule-of-inference:hypothetical-syllogism}{Hypothetical Syllogism}} \\
            &\quad\text{\aref{rule-of-inference:universal-generalization}{Universal Generalization}} \\
            &\quad\text{\cref{dfn:subset}} \\
            &\quad\text{\aref{rule-of-inference:deduction-theorem}{Deduction Theorem}} \\
        \end{aligned}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \begin{aligned}
            &\quad\text{\aref{rule-of-inference:assumption}{Assumption}}\\
            &\quad\text{\aref{rule-of-inference:assumption}{Assumption}, \cref{dfn:intersection-of-sets}}\\
            &\quad\text{\aref{rule-of-inference:universal-instantiation}{Universal Instantiation}}\\
            &\quad\text{\aref{rule-of-inference:modus-ponens}{Modus Ponens}}\\
            &\quad\text{\aref{rule-of-inference:deduction-theorem}{Deduction Theorem}}\\
            &\quad\text{\aref{rule-of-inference:universal-generalization}{Universal Generalization}, \cref{dfn:intersection-of-sets}}\\
            &\quad\text{\aref{rule-of-inference:deduction-theorem}{Deduction Theorem}}
        \end{aligned}
    \end{equation*}
    Since \( A \) and \( S \) are arbitrary, the theorem follows from \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
\end{proof}
...
\begin{proof}
    ...
    as required. \aref{rule-of-inference:universal-generalization}{Universal Generalization} completes the proof.
\end{proof}
...
\begin{proof}
    ...
    the theorem follows from \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    Since \( A \), \( B \), and \( C \) are arbitrary, the theorem follows from \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
\end{proof}
...
\begin{proof}
    ...
    \begin{equation*}
        x_{0} \in A \lor x_{0} \in B
    \end{equation*}
    By \aref{rule-of-inference:constructive-dilemma}{Constructive Dilemma}, it follows that \( x_{0} \in C \lor x_{0} \in D \).
    \begin{equation*}
        \therefore \, x_{0} \in \mleft( A \union B \mright) \implies x_{0} \in \mleft( C \union D \mright)
    \end{equation*}
    The theorem follows by \aref{rule-of-inference:universal-generalization}{Universal Generalization}
\end{proof}
...
\begin{proof}
    Let \( x \) be an arbitrary object and \( T \) be any arbitrary set. Thus, we can invoke the following inferences:
    \begin{equation*}
        \begin{aligned}[c]
            T \notin \emptyset& \\
            T \notin \emptyset \lor x \notin T& \\
    ...
    Since \( x \) is arbitrary, we conclude by \aref{rule-of-inference:universal-generalization}{Universal Generalization} that,
    \begin{equation*}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \Big]
    \end{equation*}
    By \cref{tautology:using-an-existentially-quantified-formula-to-express-the-fact-that-an-object-satisfies-a-formula},
    this becomes,
    \begin{equation*}
        \forall x \, \Big[ x \in \Union \mleft\{ A \mright\}
        \iff
...
\begin{proof}
    ...
    \end{equation*}
    Since \( x_{0} \) is arbitrary, \aref{rule-of-inference:universal-generalization}{Universal Generalization} permits us to infer,
    \begin{equation}
...
\begin{proof}
    ...
    we conclude \( x_{0} \in A \) by \aref{rule-of-inference:disjunction-elimination}{Disjunction Elimination}. Thus, \( x_{0} \in A \union \emptyset \implies x_{0} \in A \). To prove the converse, suppose that
    ...
    \end{equation*}
    The theorem follows from \aref{rule-of-inference:universal-generalization}{Universal Generalization} and \nameref{axm:axiom-of-extensionality}.
\end{proof}
...
\begin{proof}
    ...
    Since the sets \( A \), \( B \), and \( C \) are arbitrary, the theorem follows by \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
\end{proof}
...
\begin{proof}
    ...
    and \( C \) are arbitrary, the theorem follows from \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
\end{proof}
...
\begin{proof}
    ...
    Since \( A \), \( B \), and \( C \) are arbitrary, the theorem follows by \aref{rule-of-inference:universal-generalization}{Universal Generalization}.
\end{proof}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        x_{0} \in \mleft( A \symdiff B \mright) \symdiff C
    \end{equation}
    Applying \aref{rule-of-inference:disjunction-elimination}{Disjunction Elimination} to \eqref{eq:lhs-predicate-form}, \eqref{eq:left-disjunct-forward-implication},
    ...
    \( x_{0} \in A \symdiff \mleft( B \symdiff C \mright) \), we conclude by \aref{rule-of-inference:disjunction-elimination}{Disjunction Elimination} that,
    \begin{equation}
    ...
    A \symdiff \mleft( B \symdiff C \mright)\), we conclude by \aref{rule-of-inference:disjunction-elimination}{Disjunction Elimination} that,
    \begin{equation}
    ...
    \end{equation}
    Applying \aref{rule-of-inference:disjunction-elimination}{Disjunction Elimination} to \eqref{eq:rhs-predicate-form}, \eqref{eq:left-disjunct-backward-implication},
...
\begin{proof}
    ...
    \end{equation}
    Let \( x_{0} \) be an arbitrary object. Applying \aref{rule-of-inference:universal-instantiation}{Universal Instantiation} to
    ...
    \begin{equation*}
        ...
        \begin{aligned}
            ...
            &\quad \text{Definition of Conditional}\\
            &\quad \text{\aref{rule-of-inference:de-morgans-laws}{De Morgan's Laws}}\\
            &\quad \text{\cref{logical-equivalence:grouping-disjuncts-into-quantified-formula}}\\
            &\quad \text{\cref{logical-equivalence:grouping-disjuncts-into-quantified-formula}}\\
            &\quad \text{Definition of Conditional}\\
            &\quad \text{\cref{logical-equivalence:equivalent-quantified-forms-of-formulas-with-free-variables-replaced}}\\
            &\quad \text{\cref{dfn:difference}}\\
    ...
    From \eqref{eq:statement-for-union-of-T-substituted}, we infer by \aref{rule-of-inference:universal-instantiation}{Universal Instantiation} that,
    \begin{equation}
    ...
    \begin{equation*}
        ...
        \begin{aligned}
            ...
            &\quad \text{\cref{logical-equivalence:grouping-conjuncts-into-quantified-formula}}\\
            &\quad \text{\cref{logical-equivalence:grouping-conjuncts-into-quantified-formula}}\\
            &\quad \text{\cref{tautology:using-an-existentially-quantified-formula-to-express-the-fact-that-an-object-satisfies-a-formula}}\\
            &\quad \text{\cref{dfn:difference}}\\
    ...
    \end{equation*}
    And since \( x_{0} \) is arbitrary, \aref{rule-of-inference:universal-generalization}{Universal Generalization} yields,
    \begin{equation*}
...
\begin{proof}
    ...
    By \aref{rule-of-inference:disjunction-elimination}{Disjunction Elimination}, we conclude that,
    \begin{equation*}
    ...
    \end{equation}
    Let \( x_{0} \) be an arbitrary object.
    Applying \aref{rule-of-inference:universal-instantiation}{Universal Instantiation} to \eqref{eq:statement-for-the-intersection-of-T1-substituted} yields,
    \begin{equation}
...
\begin{proof}
    ...
    follows from \aref{rule-of-inference:universal-generalization}{Universal Generalization} and the \nameref{axm:axiom-of-extensionality}.
...
\begin{thm}
    \label{set-identities:set-difference-in-terms-of-symmetric-difference-and-intersection}
    For any two sets \( A \) and \( B \),
    \begin{equation*}
        A - B = A \symdiff \mleft( A \intersect B \mright)
    \end{equation*}
\end{thm}

\begin{proof}
    Let \( A \) and \( B \) be arbitrary sets. First, observe that \( A - B = A - \mleft( A \intersect B \mright) \)
    by \aref{rule-of-inference:de-morgans-laws}{De Morgan's Laws} and also that
    \( \mleft( A \intersect B \mright) - A = \emptyset \) by \cref{set-identities:right-distributivity-of-difference}.
    To express the right-hand side in terms of symmetric difference, we apply \cref{set-identities:symmetric-difference-as-the-union-of-differences}:
    \begin{align*}
        A - B
        &= A - \mleft( A \intersect B \mright)\\
        &= \mleft[ A - \mleft( A \intersect B \mright) \mright] \union \emptyset\\
        &= \mleft[ A - \mleft( A \intersect B \mright) \mright] \union \mleft[ \mleft( A \intersect B \mright) - A \mright]\\
        &= A \symdiff \mleft( A \intersect B \mright) \qedhere
    \end{align*}
\end{proof}

\begin{thm}
    \label{set-identities:union-in-terms-of-symmetric-difference-and-intersection}
    For any two sets \( A \) and \( B \),
    \begin{equation*}
        A \union B = \mleft( A \symdiff B \mright) \symdiff \mleft( A \intersect B \mright)
    \end{equation*}
\end{thm}

\begin{proof}
    Let \( A \), \( B \), and \( C \) be arbitrary sets.
    By \cref{set-identities:symmetric-difference-in-terms-of-union-and-intersection}, we have,
    \begin{equation*}
        A \symdiff B = \mleft( A \union B \mright) - \mleft( A \intersect B \mright)
    \end{equation*}
    Now consider the set \( \mleft( A \symdiff B \mright) \union \mleft( A \intersect B \mright) \).
    \begin{align*}
        \mleft( A \symdiff B \mright) \union \mleft( A \intersect B \mright)
            &= \mleft[ \mleft( A \union B \mright) - \mleft( A \intersect B \mright) \mright] \union \mleft( A \intersect B \mright)\\
            &= \mleft( A \intersect B \mright) \union \mleft[ \mleft( A \union B \mright) - \mleft( A \intersect B \mright) \mright]
    \end{align*}
    By \cref{set-identities:union-difference}, this becomes,
    \begin{align*}
        \mleft( A \symdiff B \mright) \union \mleft( A \intersect B \mright)
        &= \mleft[ \mleft( A \intersect B \mright) \union \mleft( A \union B \mright) \mright] - \mleft[ \mleft( A \intersect B \mright) - \mleft( A \intersect B \mright) \mright]\\
        &= \mleft[ \mleft( A \intersect B \mright) \union \mleft( A \union B \mright) \mright] - \emptyset\\
        &= \mleft( A \intersect B \mright) \union \mleft( A \union B \mright)
    \end{align*}
    We know that \( \union \) distributes over \( \intersect \). Thus,
    \begin{equation}
        \label{eq:union-in-terms-of-symdiff-and-intersection}
        \mleft( A \symdiff B \mright) \union \mleft( A \intersect B \mright)
        = \mleft[ A \union \mleft( A \union B \mright) \mright] \intersect \mleft[ B \union \mleft( A \union B \mright) \mright]
        = \mleft( A \union B \mright)
    \end{equation}
    The problem with \eqref{eq:union-in-terms-of-symdiff-and-intersection} is that it still contains the \( \union \) operation. To eliminate
    the \( \union \) operation, observe that \( \mleft( A \symdiff B \mright) \intersect \mleft( A \intersect B \mright) = \emptyset \). This allows
    \eqref{eq:union-in-terms-of-symdiff-and-intersection} to be re-expressed as follows:
    \begin{align*}
        \mleft( A \union B \mright)
        &= \mleft( A \symdiff B \mright) \union \mleft( A \intersect B \mright)\\
        &= \mleft[ \mleft( A \symdiff B \mright) \union \mleft( A \intersect B \mright) \mright] - \emptyset\\
        &= \mleft[ \mleft( A \symdiff B \mright) \union \mleft( A \intersect B \mright) \mright] -
        \mleft[ \mleft( A \symdiff B \mright) \intersect \mleft( A \intersect B \mright) \mright]\\
        &= \mleft( A \symdiff B \mright) \symdiff \mleft( A \intersect B \mright) \qedhere
    \end{align*}
\end{proof}

\begin{thm}
    \label{set-identities:intersection-in-terms-of-union-and-symmetric-difference}
    For any two sets \( A \) and \( B \),
    \begin{equation*}
        A \intersect B = \mleft( A \union B \mright) - \mleft( A \symdiff B \mright)
    \end{equation*}
\end{thm}

\begin{proof}
    We start with the right-hand side of the equation and apply \cref{set-identities:symmetric-difference-in-terms-of-union-and-intersection}.
    \begin{equation*}
        A \intersect B
        = \mleft( A \union B \mright) - \mleft( A \symdiff B \mright)
        = \mleft( A \union B \mright) - \mleft[ \mleft( A \union B \mright) - \mleft( A \intersect B \mright) \mright]
    \end{equation*}
    This can be further simplifed using the result from \cref{set-identities:subtract-B-minus-C-from-A}.
    \begin{equation*}
        A \intersect B = \mleft[ \mleft( A \union B \mright) - \mleft( A \union B \mright) \mright] \union
        \mleft[ \mleft( A \union B \mright) \intersect \mleft( A \intersect B \mright) \mright]
        = \emptyset \union \mleft[ \mleft( A \union B \mright) \intersect \mleft( A \intersect B \mright) \mright]
    \end{equation*}
    Since \( \intersect \) distributes over \( \union \), we have,
    \begin{equation*}
        A \intersect B = A \intersect \mleft( A \intersect B \mright) \union B \intersect \mleft( A \intersect B \mright)
        = \mleft( A \intersect B \mright) \union \mleft( A \intersect B \mright)
        = A \intersect B \qedhere
    \end{equation*}
\end{proof}
