...
\begin{proof}
    ...
    \end{equation*}
    We know that \( \emptyset \in \finiteseq{\emptyset} \) because \( \emptyset^{\emptyset} = \mleft\{ \emptyset \mright\} \) by \cref{thm:the-exponentiation-of-the-empty-set}
    and \( \emptyset \in \mleft\{ \emptyset \mright\} \).
    Now let \( x_{0} \in \finiteseq{\emptyset} \) be arbitrary.
    By definition, this means that \( x_{0} \in \emptyset^{n} \) for some \( n \in \naturalset \). But by \cref{thm:the-exponentiation-of-the-empty-set},
    \( \emptyset^{n} \neq \emptyset \) if and only if \( n = 0 = \emptyset \). Thus, \( x_{0} \in \emptyset^{\emptyset} = \mleft\{ \emptyset \mright\} \),
