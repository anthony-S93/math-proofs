...
\begin{proof}
    ...
    \end{equation}
    The laws of elementary logic (\cref{logical-equivalence:grouping-conjuncts-into-quantified-formula})
    ...
    \end{equation*}
    Using \cref{logical-equivalence:grouping-conjuncts-into-quantified-formula}, the formula becomes,
    \begin{equation*}
        y_{0} \in \Union T
        \iff
...
\begin{proof}[Proof 2.]
    ...
    \end{equation}
    Using \cref{logical-equivalence:grouping-disjuncts-into-quantified-formula}, we can simplify \eqref{eq:A-cross-B-equals-empty-final-statement}
    even further.
    \begin{align*}
        \mleft( A \times B = \emptyset \mright)
        &\iff
...
\begin{proof}
    ...
    it follows that \( z_{0} \in A \). By the \aref{rule-of-inference:deduction-theorem}{Deduction Theorem},
...
\begin{thm}
    \phantomsection \label{thm:restricted-restricted-function-is-equal-to-the-original-function-restricted-to-the-intersection}
    If \( f \) is a function and \( A \) and \( B \) are sets, then
    \begin{equation*}
        \restrictedfunction{ ( \restrictedfunctionsmall{f}{A} ) }{B} = \restrictedfunction{f}{A \,{\intersect}\, B}
    \end{equation*}
\end{thm}

\begin{proof}
    Let \( g \coloneq  \restrictedfunction{ ( \restrictedfunctionsmall{f}{A} ) }{B} \). The goal is to show that \( g \) and \( \restrictedfunctionsmall{f}{A \,{\intersect}\, B} \)
    are subsets of each other.
    Let \( x_{0} \in g \). Since \( g \) is a function, it follows that \( x_{0} \) must be an ordered pair; i.e., \( x_{0} = \orderedpair{\alpha}{\beta} \) for some
    \( \alpha \) and \( \beta \). If \( \orderedpair{\alpha}{\beta} \in g \), then \cref{dfn:restricted-function} requires that \( \orderedpair{\alpha}{\beta} \in \restrictedfunctionsmall{f}{A} \) and \( \alpha \in B \). Since \( \orderedpair{\alpha}{\beta} \in \restrictedfunctionsmall{f}{A} \), \cref{dfn:restricted-function} once again
    implies that \( \orderedpair{\alpha}{\beta} \in f \) and \( \alpha \in A \).
    This means that \( \alpha \) must be an element of both \( A \) and \( B \); i.e., \( \alpha \in A \,{\intersect}\, B \).
    Since \( \orderedpair{\alpha}{\beta} \in f \) and \( \alpha \in A \,{\intersect}\, B\), we can apply \cref{dfn:restricted-function} once again to conclude
    \( \orderedpair{\alpha}{\beta} \in \restrictedfunctionsmall{f}{A \,{\intersect}\, B} \).
    In other words, \( g \subseteq \restrictedfunctionsmall{f}{A \,{\intersect}\, B} \).

    \vspace{0.3cm}
    \noindent
    We can prove that \( \restrictedfunctionsmall{f}{A \,{\intersect}\, B} \subseteq g \) with a similar argument.
    Let \( \orderedpair{\alpha}{\beta} \in \restrictedfunctionsmall{f}{A \,{\intersect}\, B} \).
    By \cref{dfn:restricted-function}, it follows that \( \orderedpair{\alpha}{\beta} \in f \) and \( \alpha \in A \,{\intersect}\, B \).
    Clearly, \( \alpha \in A \) and \( \alpha \in B \) are both true.
    Since \( \orderedpair{\alpha}{\beta} \in f \) and \( \alpha \in A \), it follows immediately from \cref{dfn:restricted-function}
    that \( \orderedpair{\alpha}{\beta} \in \restrictedfunctionsmall{f}{A} \). But \( \alpha \in B \) is also true, so necessarily
    \( \orderedpair{\alpha}{\beta} \in g \).
\end{proof}

\begin{thm}
    \phantomsection \label{thm:restricted-function-of-the-union-of-two-compatible-functions}
    If \( f \) and \( g \) are compatible functions and \( A \) is any set, then
    \begin{equation*}
...
\begin{proof}
    Let \( F \coloneq \restrictedfunctionsmall{ \mleft( f \,{\union}\, g \mright) }{A} \) and \( G \coloneq \restrictedfunctionsmall{f}{A} \union\,\, \restrictedfunctionsmall{g}{A} \).
    We now prove that \( F \) and \( G \) are subsets of each other.

    \vspace{0.3cm}
    \noindent
    Let \( x_{0} \in F \) be arbitrary. Note that \( F \) must be a function because \( f \) and \( g \) are compatible. Thus, \( x_{0} = \orderedpair{a}{b} \)
    for some \( a \) and \( b \). Moreover, \cref{dfn:restricted-function} also requires that \( a \in A \) and \( \orderedpair{a}{b} \in \mleft( f \union g \mright) \).
    In other words, either \( \orderedpair{a}{b} \in f \) or \( \orderedpair{a}{b} \in g \).
    If \( \orderedpair{a}{b} \in f \), then \( \orderedpair{a}{b} \) must also belong to \( \restrictedfunctionsmall{f}{A} \) since \( a \in A \).
    Similarly, if \( \orderedpair{a}{b} \in g \), then \( \orderedpair{a}{b} \in \restrictedfunctionsmall{g}{A} \).
    By \aref{rule-of-inference:constructive-dilemma}{Constructive Dilemma}, we infer \( \orderedpair{a}{b} \in G \) and thus \( x_{0} \in G \).
    Therefore, \( F \subseteq G \).

    \vspace{0.3cm}
    \noindent
    Let \( x_{1} \in G \) be arbitrary. Since \( f \) and \( g \) are compatible functions, \( \restrictedfunctionsmall{f}{A} \) and \( \restrictedfunctionsmall{g}{A} \)
    must also be compatible with each other. Since \( G = \restrictedfunctionsmall{f}{A} \union \restrictedfunctionsmall{g}{A} \), it follows from
    \cref{thm:the-union-of-a-compatible-system-of-functions-is-a-function} that \( G \) is a function.
    Since \( G \) is a function, \( x_{1} \) must be an ordered pair; that is, \( x_{1} = \orderedpair{c}{d} \) for some \( c \) and \( d \).
    Furthermore, either \( \orderedpair{c}{d} \in \restrictedfunctionsmall{f}{A} \) or \( \orderedpair{c}{d} \in \restrictedfunctionsmall{g}{A} \) is true.
    If \( \orderedpair{c}{d} \in \restrictedfunctionsmall{f}{A} \), then clearly \( \orderedpair{c}{d} \in f \) and \( c \in A \) by \cref{dfn:restricted-function}.
    But if \( \orderedpair{c}{d} \in f \), then \( \orderedpair{c}{d} \) must also belong to \( f \union g \). Since \( c \in A \), it follows readily
    that \( \orderedpair{c}{d} \in F \).
    Similarly, if \( \orderedpair{c}{d} \in \restrictedfunctionsmall{g}{A} \), then necessarily \( \orderedpair{c}{d} \in g \) and \( c \in A \).
    But \( \orderedpair{c}{d} \in g \) implies \( \orderedpair{c}{d} \in f \union g \). Since \( c \in A \), it follows from \cref{dfn:restricted-function}
    that \( \orderedpair{c}{d} \in \restrictedfunctionsmall{\mleft( f \union g \mright)}{A} \).
    In other words, \( G \subseteq F \).

    \vspace{0.3cm}
    \noindent
    Since \( F \) and \( G \) are subsets of each other, they must be equal sets.
\end{proof}
